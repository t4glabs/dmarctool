...
def pass_rate_sparkline(series, threshold: float = 0.99, width: int = 640, height: int = 140,
                         colors: dict | None = None) -> str:
    """series: list of (date, total, passed, rate) as returned by analysis.daily_pass_series.

    `colors`, when given, overrides the default currentColor theming with
    literal hex -- key "ink" -- for embedding in an emailed HTML file, which
    has no access to the dashboard's live stylesheet. Default (None) is
    ...
    if len(series) < 2 or not points_with_data:
        return '<svg width="{}" height="{}"><text x="10" y="20" fill="{}" opacity="0.6">not enough data yet</text></svg>'.format(width, height, ink)
    ...
    for pct in (0.90, 0.95, 0.99, 1.00):
        ...
        gridlines.append(
            ...
            f'stroke="{ink}" stroke-opacity="0.15" stroke-width="1"/>'
        )
        gridlines.append(
            f'<text x="2" y="{gy + 4:.1f}" font-size="10" fill="{ink}" opacity="0.6">{int(pct*100)}</text>'
        )
    ...
    for i, rate in points_with_data:
        ...
        dots.append(
            f'<circle cx="{x_for(i):.1f}" cy="{y_for(rate):.1f}" r="3" fill="{ink}" opacity="0.8" '
  ...
  <line x1="{pad_l}" y1="{threshold_y:.1f}" x2="{width - pad_r}" y2="{threshold_y:.1f}"
        stroke="{ink}" stroke-opacity="0.35" stroke-width="1" stroke-dasharray="4 3"/>
