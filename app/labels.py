...
CATEGORY_LABELS = {
    ...
    "rua_unauthorized": "Report address not authorized",
    ...
    "campaign_compliance_issue": "Newsletter formatting issue",
    "subject_spam_risk": "Spam-trigger-heavy subject line",
    "content_spam_risk": "Spam-trigger-heavy newsletter content",
...
CATEGORY_HELP = {
    ...
    "lookalike_domain": "Someone has registered a domain that closely resembles this one, and set it up to handle email. DMARC protects your own domain from being forged -- it can do nothing about a different domain that merely looks like yours, which can pass every authentication check under its own settings while still fooling your supporters. Most look-alikes are innocent (unrelated businesses often own a similar name), so this is a list to review rather than an emergency.",
    "rua_unauthorized": "This domain sends its DMARC reports to an address on a different domain, and that other domain hasn't published the record that says it agrees to receive them (RFC 7489 s7.1). Reports are still arriving, so nothing is broken today -- but a receiver that enforces the rule just stops sending, with no error and no bounce, and \"no reports\" looks exactly like \"no problems\". One wildcard record on the collecting domain fixes it for every domain at once.",
    ...
    "spf_missing": "No SPF (v=spf1) TXT record was found at this domain at all -- a different problem from spf_lookup_limit (which has a record, just an overcomplicated one). Until one's published, receivers have nothing to check the sending IP against.",
    ...
    "campaign_compliance_issue": "A recent newsletter is missing something Gmail's sender guidelines require or recommend -- e.g. a working one-click unsubscribe (List-Unsubscribe/List-Unsubscribe-Post headers, mandatory for 5,000+/day senders), a Message-ID, or a subject line that misleadingly starts with \"Re:\"/\"Fwd:\".",
    "subject_spam_risk": "This newsletter's subject line scores high on a spam-trigger-word/formatting heuristic (financial or urgency bait phrases, excessive caps/punctuation/emoji). Best-effort signal, not a certainty -- worth a second look before your next send.",
    "content_spam_risk": "This newsletter's actual body content (fetched from Listmonk) scores high on the same spam-trigger-word/formatting heuristic used for subject lines. Best-effort signal, not a certainty -- worth a second look before your next send.",
...
CATEGORY_REMEDIATION = {
    ...
    "campaign_compliance_issue": "In Listmonk, check Settings -> Mailserver for unsubscribe-header configuration, and confirm each campaign's subject doesn't accidentally start with \"Re:\"/\"Fwd:\". Most of this is a one-time Listmonk configuration fix, not a per-campaign task.",
    "subject_spam_risk": "Reword the subject to describe the actual newsletter content plainly, avoiding urgency/financial bait phrasing, ALL CAPS, and excessive punctuation or emoji -- see the flagged words/phrases in the detail above for exactly what to change.",
    "content_spam_risk": "In Listmonk, edit this campaign's body to remove or reword the flagged phrases/formatting -- see the detail above for exactly what to change.",
    ...
    "lookalike_domain": "Look at each one and decide: is it an unrelated business that happens to own a similar name, or is it imitating you? For the innocent ones, press \"Mark as known\" so they stop being counted. For a genuine imitation, the options are to report it to the registrar (look up who registered it, then use their abuse contact), warn your supporters that you only ever email from your real domain, and -- if the name matters to your organization -- register the closest variants yourself so nobody else can. There is nothing to change in your own DNS for this.",
    "rua_unauthorized": "Publish ONE wildcard TXT record on the domain that collects your reports and every tracked domain is covered at once: name `*._report._dmarc.<collecting domain>`, value `v=DMARC1`. For example, if all your domains report to dmarc-reports@aikyamfellows.org, add `*._report._dmarc.aikyamfellows.org` with the value `v=DMARC1` in aikyamfellows.org's DNS. Per-domain records (`<reporting domain>._report._dmarc.<collecting domain>`) also work but you'd need one for every domain. Nothing is broken until you do this -- reports are still arriving -- so it's housekeeping, not an emergency.",
...
CLASSIFICATION_LABELS = {
    ...
    "mailgun": "Bulk/newsletter sender (Mailgun)",
...
CLASSIFICATION_HELP = {
    ...
    "mailgun": "This sender's reverse DNS (PTR) resolves to Mailgun's own mailgun.net infrastructure.",
...
SETTINGS_META = {
    ...
    "dkim_alignment_min_volume": {
        "label": "DKIM alignment check -- minimum mail volume",
        "help": "Below this many messages in the window, we don't have enough data to trust the DKIM-alignment rate.",
        "example": "20 means we want at least 20 messages before judging whether DKIM is aligning.",
    },
    "dkim_alignment_window_days": {
        "label": "DKIM alignment check -- lookback window (days)",
        "help": "How many days of recent reports to look at when measuring what fraction of mail has DKIM actually aligning.",
        "example": "30 means the last month of reports.",
    },
    "dkim_alignment_min_rate": {
        "label": "DKIM alignment check -- minimum acceptable rate",
        "help": "Below this fraction of mail having DKIM actually align (the receiver's own verdict, not just SPF), flag it -- DMARC only needs one of SPF or DKIM to pass, so a domain can look healthy overall while having no DKIM fallback at all.",
        "example": "0.5 means flag it if less than half your mail has DKIM aligning.",
    },
...
SETTINGS_GROUPS = [
    ...
    ("🔐 Authentication compliance (SPF/DKIM)", [
        ...
        "dkim_alignment_min_volume", "dkim_alignment_window_days", "dkim_alignment_min_rate",
    ]),
    ("📨 Mailgun", [
